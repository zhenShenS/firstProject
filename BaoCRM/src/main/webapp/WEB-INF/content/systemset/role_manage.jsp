...
<table id="data_list_table" class="data_list_table" cellSpacing=1 cellPadding=1>
	...
	<c:forEach items="${sessionScope.sysRole}" var="sysRole">
	<tr>
		<td class="list_data_number">${sysRole.roleId}</td>
		<td class="list_data_ltext" >${sysRole.roleName}</td>
		<td class="list_data_ltext" >${sysRole.roleDesc}</td>
		...
		<td class="list_data_op">
			<img onClick="to('systemset/${sysRole.roleId}/role_update');" 
			                  title="编辑" 
			                  src="images/bt_edit.gif" 
			                  class="op_button" />
			<img onClick="to('systemset/${sysRole.roleId}/role_delete');"
							  title="删除" 
