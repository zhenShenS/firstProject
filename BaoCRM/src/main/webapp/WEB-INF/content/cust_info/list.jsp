...
<body>
	...
	<form action="cust/1/selectCustomer" method="post">
			...
			<tr>
				<td width=100% height=32 align=center colspan=6 bgcolor=#ffffff>
					<a href="help"><input class="common_button" type="button"
						value="帮助"></a>&nbsp;&nbsp;&nbsp; <a href="add_customer"><input
						class="common_button" type="button" value="新建"></a>&nbsp;&nbsp;&nbsp;
	...
	<table id="data_list_table" class="data_list_table" cellSpacing=1
		...
		<%-- 	<tr>
				...
				<td>${page.custId}</td>
				<td>${page.custName}</td>
				<td>${page.custRegion}</td>
				<td>${page.custManagerName}</td>
				<td>${page.custLevel}</td>
			</tr> --%>
			<tr>
				<td class="list_data_number">${i.count}</td>
				<td class="list_data_text">${page.custId}</td>
				<td class="list_data_ltext">${page.custName}</td>
				<td class="list_data_text">${page.custRegion}</td>
				<td class="list_data_text">${page.custManagerName}</td>
				<td class="list_data_text">${page.custLevelLabel}</td>
				<td class="list_data_op"><img
					onClick="to('list/${page.custId}/update');" title="编辑"
					src="images/bt_edit.gif" class="op_button" /> <img title="编辑"
					src="images/bt_edit.png" class="op_button" /> <img
					onClick="to('controller/${page.custId}/linkMan');" title="联系人"
					src="images/bt_linkman.gif" class="op_button" /> <img
					onClick="to('controller/${page.custNo}/Communication');" title="交往记录"
					src="images/bt_orders.gif"> <!-- 经理只能删除属于自己的客户 --> <a
					href="javascript:if(confirm('确认删除？')){window.location.href='cust/${page.custId}/deleteCustomr'}">
						<img title="删除" src="images/bt_del.gif" class="op_button" />
				</a> <img title="删除" src="images/bt_del.png" class="op_button" /></td>
			</tr>
		</c:forEach>

		<tr>
			<th colspan="100" class="pager">共${sessionScope.pageInfo.total}条记录
				每页 4 条 第页 <a href="cust/1/findAllCust">首页</a> <c:choose>
					<c:when test="${sessionScope.pageInfo.hasPreviousPage}">
						<a href="cust/${sessionScope.pageInfo.prePage}/findAllCust">上一页</a>
					</c:when>
					<c:otherwise>
				...
				<c:choose>
					<c:when test="${sessionScope.pageInfo.hasNextPage}">
						<a href="cust/${sessionScope.pageInfo.nextPage}/findAllCust">下一页</a>
					</c:when>
					<c:otherwise>
						<font color="#ABA8AB">下一页</font>
					</c:otherwise>
				</c:choose> <a href="cust/${sessionScope.pageInfo.lastPage}/findAllCust">尾页</a>
			</th>
		</tr>
	</table>
</body>
